...
def test_autoplot_group_type():
    ...
    figs = vplot.auto_plot(path, group="type", show=False)
    assert len(figs) > 0
    for fig in figs:
        assert isinstance(fig, matplotlib.figure.Figure)
        matplotlib.pyplot.close(fig)
...
def test_autoplot_group_param():
    ...
    figs = vplot.auto_plot(path, group="param", show=False)
    assert len(figs) > 0
    for fig in figs:
        assert isinstance(fig, matplotlib.figure.Figure)
        matplotlib.pyplot.close(fig)
...
def test_autoplot_group_none():
    ...
    figs = vplot.auto_plot(path, group="none", show=False)
    assert len(figs) > 0
    for fig in figs:
        assert isinstance(fig, matplotlib.figure.Figure)
        matplotlib.pyplot.close(fig)
...
def test_autoplot_filter_bodies():
    ...
    figs = vplot.auto_plot(path, bodies=["cbp"], show=False)
    assert len(figs) > 0
...
def test_autoplot_filter_params():
    ...
    figs = vplot.auto_plot(path, params=["Eccentricity"], show=False)
    assert len(figs) > 0
...
def test_autoplot_invalid_group():
    ...
    with pytest.raises(AssertionError, match="must be one of"):
        vplot.auto_plot(path, group="invalid", show=False)


def test_autoplot_xlog():
    """Test xlog parameter."""
    figs = vplot.auto_plot(path, xlog=True, show=False)
    assert len(figs) > 0
...
def test_autoplot_ylog():
    """Test ylog parameter."""
    figs = vplot.auto_plot(path, ylog=True, show=False)
    assert len(figs) > 0
...
def test_autoplot_figsize():
    ...
    figs = vplot.auto_plot(path, figsize=(10, 8), show=False)
    assert len(figs) > 0
...
def test_autoplot_no_params_found():
    ...
    with pytest.raises(RuntimeError, match="No parameters found for plotting"):
        vplot.auto_plot(path, params=["NonExistentParameter"], show=False)
...
def test_autoplot_show_true():
    ...
    with patch('matplotlib.pyplot.show'):
        ...
        result = vplot.auto_plot(path, group="param", show=True)
